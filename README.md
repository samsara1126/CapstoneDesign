...
## 주요 기능 
* **이미지 판별:** 사람의 얼굴 특징을 분석하여 실제 인물인지 AI가 생성한 인물인지 판별 (Deepfake 탐지).
* **텍스트 판별:** 뉴스 기사 등의 텍스트 패턴과 분포를 분석하여 AI 생성 여부(Real/Fake) 판별.
* **웹 기반 서비스:** 별도의 설치 없이 웹 브라우저에서 바로 파일 업로드 및 결과 확인 가능.
* ~~**로그인 및 대시보드(미구현):** 로그인 후 판별 결과에 대한 신뢰도 점수 저장, 시각화 제공.~~
...
## 추후 개선 사항
현재 시스템은 이미지와 텍스트를 독립적으로 판별하고 있어, 두 모달리티 간의 정합성 검증 기능을 추가하여 맥락적 조작까지 탐지하는 것을 목표로 합니다.
* **범용 이미지 판별로의 도메인 확장:**
    * **현재 한계:** 현재 모델은 딥페이크 탐지를 목적으로 사람의 얼굴 특징과 아티팩트를 집중적으로 학습하여, 얼굴이 없는 풍경, 사물, 예술 작품 등의 이미지에 대해서는 판별 성능이 제한적일 수 있습니다.
    * **개선 방향:** 인물 사진뿐만 아니라 다양한 도메인(풍경, 사물 등)의 AI 생성 이미지 데이터셋을 추가 확보 및 학습시켜, 특정 피사체에 국한되지 않는 범용적인 이미지 위변조 판별 시스템으로 확장할 계획입니다.

* **이미지 모델 경량화 및 서버리스 전환:**
...
* **팩트 체크 기능 보완:**
    * **현재 한계:** 현재 텍스트 모델은 '누가 작성했는가(AI vs 인간)'를 판별하는 데 특화되어 있어, 내용의 사실 여부 자체는 검증하지 않습니다.
